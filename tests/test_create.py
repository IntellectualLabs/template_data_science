...
def test_bake_selecting_license(cookies):
    ...
    for project_license, (
        ...
        setup_subtext,
        should_contain_year,
    ) in license_strings.items():
        with bake_in_temp_dir(
            cookies, extra_context={"open_source_license": project_license}
        ) as result:
...
def test_bake_package_name(cookies):
    with bake_in_temp_dir(
        cookies, extra_context={"package_name": "my_package"}
    ) as result:
        with inside_dir(result.project):
            ...
            "from my_package import examplemodule" in open(
                os.path.join(
                    "tests", "my_package", "examplemodule", "test_add_value_to_numpy.py"
                )
            ).read()
            "from my_package import examplemodule" in open(
                os.path.join(
                    "tests", "my_package", "examplemodule", "test_hello_world.py"
                )
            ).read()
